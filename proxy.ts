...
export async function proxy(request: NextRequest) {
  ...
  // --- Employee: no ERP access (no-login role, redirect to login) ---
  if (role === "employee") {
  ...
  // --- Foreman + Office + Accountant: check module permissions ---
  if (role === "foreman" || role === "office" || role === "accountant") {
    ...
    // Check module permissions for specific routes
    const matchedRoute = Object.keys(moduleRoutes).find((route) =>
      pathname.startsWith(route)
    )
    if (matchedRoute) {
      const moduleName = moduleRoutes[matchedRoute]
      const isWriteRoute = pathname.endsWith("/neu") || pathname.includes("/bearbeiten")
